...
namespace mRemoteNG.UI.Window
{
	public class ScreenshotManagerWindow : BaseWindow
	{
		...
		private void InitializeComponent()
		{
			...
			//flpScreenshots
			//
			flpScreenshots.Anchor = (AnchorStyles) (((AnchorStyles.Top | AnchorStyles.Bottom) 
				| AnchorStyles.Left) 
				| AnchorStyles.Right);
			...
			//msMain
			//
			msMain.Font = new Font("Microsoft Sans Serif", (float) (8.25F), FontStyle.Regular, GraphicsUnit.Point, Convert.ToByte(0));
		...
		private void pbScreenshotOpen_MouseDown(Object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
				CloseOpenedScreenshot((Form)((Control)sender).Parent);
			}
		}
		...
		private void OpenScreenshot(PictureBox sender)
		{
			try
			{
				...
				nPB.Anchor = (AnchorStyles) (AnchorStyles.Left | AnchorStyles.Bottom | AnchorStyles.Right | AnchorStyles.Top);
				nPB.Image = mImage;
				nPB.ContextMenuStrip = cMenScreenshot;
				nPB.Show();
		...
		private void SaveSingleImage()
		{
			try
			{
				...
				{
					switch (dlgSaveSingleImage.FileName.Substring(dlgSaveSingleImage.FileName.LastIndexOf(".") + 1).ToLower())
					{
						case "gif":
							(cMenScreenshot.Tag as PictureBox).Image.Save(dlgSaveSingleImage.FileName, System.Drawing.Imaging.ImageFormat.Gif);
							break;
						case "jpeg":
							(cMenScreenshot.Tag as PictureBox).Image.Save(dlgSaveSingleImage.FileName, System.Drawing.Imaging.ImageFormat.Jpeg);
							break;
						case "jpg":
							(cMenScreenshot.Tag as PictureBox).Image.Save(dlgSaveSingleImage.FileName, System.Drawing.Imaging.ImageFormat.Jpeg);
							break;
						case "png":
							(cMenScreenshot.Tag as PictureBox).Image.Save(dlgSaveSingleImage.FileName, System.Drawing.Imaging.ImageFormat.Png);
							break;
					}
				}
			}
